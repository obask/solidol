...
import { sharedConfig } from "../render/hydration.js";


import {ComponentProps, ValidComponent, enableHydration, splitProps} from "../render/component"

export * from "./client.js";

import {Suspense, SuspenseList} from "solidol/render/Suspense"
 ...
 *
 * Useful for inserting modals and tooltips outside a cropping layout. If no mount point is given,
 * the portal is inserted in document.body; it is wrapped in a `<div>` unless the target is `document.head`
 * or `isSVG` is true. setting `useShadow` to true places the element in a shadow root to isolate styles.
 *
...
export function Portal<T extends boolean = false, S extends boolean = false>(props: {
  ...
  if (mount instanceof HTMLHeadElement) {
    ...
    createRoot((dispose: any) => insert(mount, () => (!clean() ? renderPortal()() : dispose()), null));
    onCleanup(() => {
